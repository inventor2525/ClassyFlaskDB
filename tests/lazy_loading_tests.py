...
class LazyLoadingTests(unittest.TestCase):
	...
	def test_list_lazy_loading(self):
		...
		for engine_type in range(0, 2):
			...
			for index, item in enumerate(queried.items):
				...
				self.assertFalse(isinstance(item, InstrumentedValue))
				prev_index = index
			...
			for item, queried_item in zip(items, queried.items):
				...
				self.assertFalse(isinstance(queried_item, InstrumentedValue))
	...
	def test_complex_dict_lazy_loading(self):
		...
		for engine_type in range(0, 2):
			...
			container, queried, Person, Family = setup_and_query(engine_type)
			values = list(queried.families.values())
			self.assertEqual(values[0].last_name, "Smith")
			self.assertEqual(values[1].last_name, "Doe")
			
			container, queried, Person, Family = setup_and_query(engine_type)
			for index,key in enumerate(queried.families):
				self.assertEquals(key, ['smith', 'doe'][index])
			
			container, queried, Person, Family = setup_and_query(engine_type)
			for orig,key in zip(['smith', 'doe'], queried.families):
